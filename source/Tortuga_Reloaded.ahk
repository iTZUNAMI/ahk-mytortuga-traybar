...
Menu, mOpzioni, Add, Controlla Versione Client, ContrWeb
Menu, mOpzioni, Add, Suggerisci Sito Web, SuggWeb
Menu, mOpzioni, Add, Info, About..
Menu, Tray, Add, Opzioni, :mOpzioni
Menu, Tray, Add,Aggiorna Lista,AggiornaLista
AutoStart:="on"
...
IfNotExist,  %A_Temp%\%NomeProgramma%_config.json
  {
  UrlDownloadToFile, https://dl.dropboxusercontent.com/u/2750107/mytortuga/web/myTRG_config.json,  %A_Temp%\%NomeProgramma%_config.json
  }
...
;se prima volta in assoluto mostra tooltips benvenuto

IfNotExist,  %A_ScriptDir%\%NomeProgramma%_e
{
TrayTip, Benvenuto su myToRTuGa, Per iniziare cliccate il tasto destro sull'icona.`n, 30, 17

;creo collegamento sul menu start
FileCreateShortcut, %A_ScriptFullPath%, %A_StartMenu%\%NomeProgramma%.lnk, %A_ScriptDir% 
}
...
IfExist,  %A_ScriptDir%\%NomeProgramma%_d
{
; questo file viene creato disabilitando auto avvio
AutoStart:="off"
}
IfExist,  %A_ScriptDir%\%NomeProgramma%_e
{
...
else if (AutoStart="on")
{
  ...
  FileAppend, Imposta Start, %A_ScriptDir%\%NomeProgramma%_e ; avvio enabled config file
}
...
;WIN Z mostra menu dove si trova mouse
#z::Menu, Tray, Show 
...
ContrWeb:
{
ToURLD("http://www.mytrg.info/index.php?v=13")
}
return
...
SuggWeb:
{
  ToURLD("http://www.mytrg.info/index.php")
}
return
...
msgbox,64,Info,
(LTrim
	%NomeProgramma% - myToRTuGa Tray Bar
	
    Questa Tray Bar vi permette un rapido accesso ai miglior siti italiani.
	
    Tutti i siti web vengono aggiornati in automatico ad ogni avvio.
	Potete aggiornare anche manualmente la lista cliccando Aggiorna Lista.
	
	Non dovrete più preoccuparvi di blocchi/censure/cambi di dominio su questi siti web.
	Ogni tanto controllate se ci sono nuove versioni del Client per nuove opzioni.
	
	
	HotKey: Tasto rapido della tastiera -> Win + Z
...
IfExist, %A_Startup%\%NomeProgramma%.lnk
{
  ...
  ; msgbox eliminato
  FileAppend, Imposta Start, %A_ScriptDir%\%NomeProgramma%_d
...
FileCreateShortcut, %A_ScriptFullPath%, %A_Startup%\%NomeProgramma%.lnk, %A_ScriptDir% 
;msgbox riaggiunto
FileAppend, Imposta Start, %A_ScriptDir%\%NomeProgramma%_e
Filedelete, %A_ScriptDir%\%NomeProgramma%_d
return
}
